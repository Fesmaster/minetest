...
// BEGIN PATHFIX CHANGE: Author: Fesmaster
#include <sstream>
// END PATHFIX CHANGE
...
class debugger {
 ...
 private:
  ...
  // BEGIN PATHFIX CHANGE: Author: Fesmaster
  //get a array of the file path parts. index 0 is the file name, the last element is the root or base folder.
  static void get_filepath_hash_array(const char* path, std::vector<uint64_t>* out_array)
  {
      std::vector<std::string> tmp;
      std::string path_str(path);
      //fix up inconsistant path seperators
      for (char& c : path_str)
      {
          if (c == '\\')
          {
      ...
      //clean up the array (ie, remove "." and ".." parts and the ones they effect
      out_array->clear();
      int ignore_count = 0;
      for (auto i = tmp.rbegin(); i != tmp.rend(); i++)
      {
          ...
          if (*i != ".")
          {
              ...
              else
              {
                  ...
                  out_array->push_back(std::hash<std::string>{}(*i));
              }
          }
      }
  }

  static std::unordered_map<std::string, uint64_t> known_filenames;
  ...
  static bool is_file_path_match(const char* path1, const char* path2) {
    ...
    */
      //caching file names for faster response time
      std::string strpath1(path1);
      std::string strpath2(path2);
      auto f1 = known_filenames.find(strpath1);
      auto f2 = known_filenames.find(strpath2);
      if (f1 != known_filenames.end() && f2 != known_filenames.end())
      {
      ...
      // ok, fine. Take the slow path
      std::vector<uint64_t> filePath1;
      std::vector<uint64_t> filePath2;
      get_filepath_hash_array(path1, &filePath1);
      get_filepath_hash_array(path2, &filePath2);
      int i = 0, j = 0;
      while (i < filePath1.size() && j < filePath2.size())
      {
          if (filePath1[i] != filePath2[j])
          {
              //ensure that if either of these files do not have a hash, they get one.
              if (f1 == known_filenames.end())
              {
                  known_filenames[strpath1] = std::hash<std::string>{}(strpath1);
              }
              if (f2 == known_filenames.end())
              {
                  known_filenames[strpath2] = std::hash<std::string>{}(strpath2);
              }
              return false;
          }
          i++;
      ...
      //well, fast path failed. Make sure it works next time.
      uint64_t hash = 0;
      if (f1 != known_filenames.end())
      {
          hash = f1->second;
      }
      else if (f2 != known_filenames.end())
      {
          hash = f2->second;
      }
      else
      {
          hash = std::hash<std::string>{}(strpath1);
      }
      known_filenames[strpath1] = hash;
      known_filenames[strpath2] = hash;
      //return true when one runs out with no equality because one path may be absolute, one relative
      return true;
  }
...
#if LRDB_IMPLEMENTATION
std::unordered_map<std::string, uint64_t> debugger::known_filenames;
#endif
